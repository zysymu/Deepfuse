...
class Scanner(object):
    ...
    def __init__(self, filename, stamp_size, mode, img_hdu=1):
        """    
        Parameters
        ----------   
        filename: str
        ...
        mode: str, "real" or "simulation-x"
            Mode of operation. If set to "real", use DeepScan to extract sources from real astronomical data that can be located anywhere around the input image.
            If set to "simulation-x", use DeepScan to detect sources on the input image but extract only the largest body located around the middle of the image,
            around an area of size x pixels. For example: simulation-15 extracts the largest source detected around a square of 15x15 pixels centered in the
            middle of the image.
    ...
    def save_stamps(self, dir_name, catalog=True):
        """
        ...
        catalog: bool, deafult True
            If set to "True", will create a directory with stamps and catalog. If set to "False", will save the stamps directly in dir_name.
        ...
        for index in range(len(stamps)):
            ...
            if catalog:
                ...
                stamp_filename = os.path.join(dir_name + ".fits")
    ...
    def _get_stamps(self):
        ...
        if self.mode == "real":
            assert df.empty != True, "DataFrame is empty: no sources found!"
            ...
            df = df[(df["ycen"] >= y_pad[0]) & (df["ycen"] <= y_pad[1])]

            # select only the largest source
            df = df[df["R50"] == df["R50"].max()]

            assert df.empty != True, "DataFrame is empty: no sources found!"
            
            stamp = Cutout2D(self.data, position=(df["xcen"].values, df["ycen"].values), size=(self.stamp_size, self.stamp_size), wcs=w, copy=True)
            stamps.append(stamp)
            header_new = stamp.wcs.to_header()
            headers.append(header_new)

            df["name"] = str(self.filename.rsplit(".", 1)[0].rsplit("/")[-1]) # name of the output .fits image
            df["association"] = str(self.filename.rsplit(".", 1)[0].rsplit("/")[-2]) # name of the big cutout image
            df["simulation"] = str(self.filename.rsplit(".", 1)[0].rsplit("/")[-3]) # name of the mosaic
